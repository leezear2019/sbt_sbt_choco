 ...
 * @since 21/09/12
 */
public class ImpactBasedOpt extends AbstractStrategy<IntVar> implements IMonitorDownBranch, IMonitorContradiction, ICause {
    ...
    private boolean learnsAndFails; // does the learning phase leads to a failure
     ...
     */
    public ImpactBasedOpt(IntVar[] ivariables, int alpha, int split, int nodeImpact, long seed, boolean initOnly) { //TODO: node impacts
        super(ivariables);
    ...
    public ImpactBasedOpt(IntVar[] vars, boolean initOnly) {
...
//            // 随机选择
//            int i = random.nextInt(variable.getDomainSize());
...
//            }
    ...
    public Decision<IntVar> getDecision() {
        ...
        if (bests.size() > 0) {
            ...
            currentVarSize = best.getDomainSize();
        }
...
//                                System.out.printf("(%2d,%2d) init impact is %.2f\n", i, j, im);
                            }
                        }
                        it.dispose();
                    }
...
//            solver.getEngine().fails(this, lAfVar, "Impact::init:: detect failures");
        ...
        if (left) {
            if (currentVar > -1) { // if the decision was computed by another strategy
                if (asgntFailed) {
                    ...
                    double filterSpaceSize = searchSpaceSize.get() / currentVarSize;
                    updateImpact(sssz / filterSpaceSize, currentVar, currentVal);
    ...
    private double computeImpactB(IntVar v, int a, int b, double before) {
        ...
        try {
            ...
            after = searchSpaceSize(vars);
            // 只考虑传播缩减的搜索空间，不考虑赋值导致的
            int sizeVA = v.getDomainSize();
            return 1.0d - ((after / sizeVA) / (before / sizeVB));
        } catch (ContradictionException e) {
            model.getSolver().getEngine().flush();
            model.getEnvironment().worldPop();
            model.getEnvironment().worldPush();
    ...
    private void updateImpact(double nImpact, int varIdx, int valIdx) {
        ...
        double impact = Ilabel[varIdx][valIdx];
        if (nImpact > impact) {
            impact *= (aging - 1);
            impact += nImpact;
            impact /= aging;
